...
router.get('/getall/:searchTerm?', admin, auth, handler(async (req, res) => {
  const {
    searchTerm
  } = req.params || "";
...
router.put('/toggleBlock/:userId', admin, auth, handler(async (req, res) => {
  const {
    userId
  } = req.params;
...
router.get('/getById/:userId', auth, admin, handler(async (req, res) => {
  const {
    userId
  } = req.params;
  const user = await UserModel.findById(userId, {
...
router.put('/update', auth, admin, handler(async (req, res) => {
  const {
    id,
    name,
    email,
